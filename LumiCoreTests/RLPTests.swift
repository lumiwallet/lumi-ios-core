//
//  RLPTests.swift
//  LumiCoreTests
//
//  Copyright © 2020 LUMI WALLET LTD. All rights reserved.
//

import XCTest
@testable import LumiCore


class RLPTests: XCTestCase {
    
    func testDecode() {
        let data = Data(hex: "")
        
        do {
            let _: Any = try RLP.decode(data)
        } catch {
            XCTAssert(false, "\(error)")
        }
    }
    
    func testString() {
        let data = getTestStringData()
        for item in data {
            let input = item.0.data(using: .utf8)
            let output = item.1
            let rlp = try! RLP.encode(input)
            XCTAssertEqual(rlp, Data(hex: output))
            let _: Data = try! RLP.decode(rlp)
        }
    }

    func testList() {
        let data = getTestListData()
        for item in data {
            let input = item.0
            let output = item.1
            let rlp = try! RLP.encode(input)
            XCTAssertEqual(rlp, Data(hex: output))
            let _: [Any] = try! RLP.decode(rlp)
        }
    }

    func getTestStringData() -> [(String, String)] {
        let data = [ ("", "0x80"),
                     ("dog", "0x83646f67"),
                     ("Lorem ipsum dolor sit amet, consectetur adipisicing eli", "0xb74c6f72656d20697073756d20646f6c6f722073697420616d65742c20636f6e7365637465747572206164697069736963696e6720656c69"),
                     ("Lorem ipsum dolor sit amet, consectetur adipisicing elit", "0xb8384c6f72656d20697073756d20646f6c6f722073697420616d65742c20636f6e7365637465747572206164697069736963696e6720656c6974"),
                     ("Lorem ipsum dolor sit amet, consectetur adipiscing elit. Curabitur mauris magna, suscipit sed vehicula non, iaculis faucibus tortor. Proin suscipit ultricies malesuada. Duis tortor elit, dictum quis tristique eu, ultrices at risus. Morbi a est imperdiet mi ullamcorper aliquet suscipit nec lorem. Aenean quis leo mollis, vulputate elit varius, consequat enim. Nulla ultrices turpis justo, et posuere urna consectetur nec. Proin non convallis metus. Donec tempor ipsum in mauris congue sollicitudin. Vestibulum ante ipsum primis in faucibus orci luctus et ultrices posuere cubilia Curae; Suspendisse convallis sem vel massa faucibus, eget lacinia lacus tempor. Nulla quis ultricies purus. Proin auctor rhoncus nibh condimentum mollis. Aliquam consequat enim at metus luctus, a eleifend purus egestas. Curabitur at nibh metus. Nam bibendum, neque at auctor tristique, lorem libero aliquet arcu, non interdum tellus lectus sit amet eros. Cras rhoncus, metus ac ornare cursus, dolor justo ultrices metus, at ullamcorper volutpat", "")]
        return data
    }

    func getTestListData() -> [(Any, String)] {
        let data: [(Any, String)] = [([], "0xc0"),
                                     (["dog".data(using: .utf8), "god".data(using: .utf8), "cat".data(using: .utf8)], "0xcc83646f6783676f6483636174"),
                                     ([ "asdf".data(using: .utf8),
                                        "qwer".data(using: .utf8),
                                        "zxcv".data(using: .utf8),
                                        "asdf".data(using: .utf8),
                                        "qwer".data(using: .utf8),
                                        "zxcv".data(using: .utf8),
                                        "asdf".data(using: .utf8),
                                        "qwer".data(using: .utf8),
                                        "zxcv".data(using: .utf8),
                                        "asdf".data(using: .utf8),
                                        "qwer".data(using: .utf8)
                                     ], "0xf784617364668471776572847a78637684617364668471776572847a78637684617364668471776572847a78637684617364668471776572"),
                                     ([
                                        ["asdf".data(using: .utf8), "qwer".data(using: .utf8), "zxcv".data(using: .utf8)],
                                        ["asdf".data(using: .utf8), "qwer".data(using: .utf8), "zxcv".data(using: .utf8)],
                                        ["asdf".data(using: .utf8), "qwer".data(using: .utf8), "zxcv".data(using: .utf8)],
                                        ["asdf".data(using: .utf8), "qwer".data(using: .utf8), "zxcv".data(using: .utf8)]
                                     ], "0xf840cf84617364668471776572847a786376cf84617364668471776572847a786376cf84617364668471776572847a786376cf84617364668471776572847a786376"),
                                     ([
                                        ["asdf".data(using: .utf8), "qwer".data(using: .utf8), "zxcv".data(using: .utf8)],
                                        ["asdf".data(using: .utf8), "qwer".data(using: .utf8), "zxcv".data(using: .utf8)],
                                        ["asdf".data(using: .utf8), "qwer".data(using: .utf8), "zxcv".data(using: .utf8)],
                                        ["asdf".data(using: .utf8), "qwer".data(using: .utf8), "zxcv".data(using: .utf8)],
                                        ["asdf".data(using: .utf8), "qwer".data(using: .utf8), "zxcv".data(using: .utf8)],
                                        ["asdf".data(using: .utf8), "qwer".data(using: .utf8), "zxcv".data(using: .utf8)],
                                        ["asdf".data(using: .utf8), "qwer".data(using: .utf8), "zxcv".data(using: .utf8)],
                                        ["asdf".data(using: .utf8), "qwer".data(using: .utf8), "zxcv".data(using: .utf8)],
                                        ["asdf".data(using: .utf8), "qwer".data(using: .utf8), "zxcv".data(using: .utf8)],
                                        ["asdf".data(using: .utf8), "qwer".data(using: .utf8), "zxcv".data(using: .utf8)],
                                        ["asdf".data(using: .utf8), "qwer".data(using: .utf8), "zxcv".data(using: .utf8)],
                                        ["asdf".data(using: .utf8), "qwer".data(using: .utf8), "zxcv".data(using: .utf8)],
                                        ["asdf".data(using: .utf8), "qwer".data(using: .utf8), "zxcv".data(using: .utf8)],
                                        ["asdf".data(using: .utf8), "qwer".data(using: .utf8), "zxcv".data(using: .utf8)],
                                        ["asdf".data(using: .utf8), "qwer".data(using: .utf8), "zxcv".data(using: .utf8)],
                                        ["asdf".data(using: .utf8), "qwer".data(using: .utf8), "zxcv".data(using: .utf8)],
                                        ["asdf".data(using: .utf8), "qwer".data(using: .utf8), "zxcv".data(using: .utf8)],
                                        ["asdf".data(using: .utf8), "qwer".data(using: .utf8), "zxcv".data(using: .utf8)],
                                        ["asdf".data(using: .utf8), "qwer".data(using: .utf8), "zxcv".data(using: .utf8)],
                                        ["asdf".data(using: .utf8), "qwer".data(using: .utf8), "zxcv".data(using: .utf8)],
                                        ["asdf".data(using: .utf8), "qwer".data(using: .utf8), "zxcv".data(using: .utf8)],
                                        ["asdf".data(using: .utf8), "qwer".data(using: .utf8), "zxcv".data(using: .utf8)],
                                        ["asdf".data(using: .utf8), "qwer".data(using: .utf8), "zxcv".data(using: .utf8)],
                                        ["asdf".data(using: .utf8), "qwer".data(using: .utf8), "zxcv".data(using: .utf8)],
                                        ["asdf".data(using: .utf8), "qwer".data(using: .utf8), "zxcv".data(using: .utf8)],
                                        ["asdf".data(using: .utf8), "qwer".data(using: .utf8), "zxcv".data(using: .utf8)],
                                        ["asdf".data(using: .utf8), "qwer".data(using: .utf8), "zxcv".data(using: .utf8)],
                                        ["asdf".data(using: .utf8), "qwer".data(using: .utf8), "zxcv".data(using: .utf8)],
                                        ["asdf".data(using: .utf8), "qwer".data(using: .utf8), "zxcv".data(using: .utf8)],
                                        ["asdf".data(using: .utf8), "qwer".data(using: .utf8), "zxcv".data(using: .utf8)],
                                        ["asdf".data(using: .utf8), "qwer".data(using: .utf8), "zxcv".data(using: .utf8)],
                                        ["asdf".data(using: .utf8), "qwer".data(using: .utf8), "zxcv".data(using: .utf8)]
                                     ], ""),
                                     ([[[], []], []], "0xc4c2c0c0c0"),
                                     ([[], [[]], [[], [[]]]], "0xc7c0c1c0c3c0c1c0"),
                                     ([["key1".data(using: .utf8), "val1".data(using: .utf8)], ["key2".data(using: .utf8), "val2".data(using: .utf8)], ["key3".data(using: .utf8), "val3".data(using: .utf8)], ["key4".data(using: .utf8), "val4".data(using: .utf8)]], "0xecca846b6579318476616c31ca846b6579328476616c32ca846b6579338476616c33ca846b6579348476616c34")]
        return data
    }
}
